...
*  SPI.
*/
    unsigned char TxBuff[] = {c};

    // Set DC to "LOW", to signal to the OLED that a command byte
    // is about to be sent.
    GPIOPinWrite(GPIOA0_BASE, GPIO_PIN_7, 0);

    // Set CS bit for OLED to "LOW", to signal beginning of communication.
    GPIOPinWrite(GPIOA0_BASE, GPIO_PIN_6, 0);

    // Send the char to OLED.
    MAP_SPITransfer(GSPI_BASE, TxBuff, NULL, 1, SPI_CS_ENABLE|SPI_CS_DISABLE);

    // Set CS bit for OLED to "HIGH", to signal end of communication.
...
*/
    ...
    // Set DC to "HIGH", to signal to the OLED that a data byte
    // is about to be sent.
